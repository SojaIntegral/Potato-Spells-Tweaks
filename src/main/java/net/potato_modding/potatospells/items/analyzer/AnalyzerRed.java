...
public class AnalyzerRed extends ImbuableCurio {
    ...
    public AnalyzerRed() {
        super(ItemPropertiesHelper.equipment().stacksTo(1).rarity(Rarity.EPIC),
                "head", SpellDataRegistryHolder.of(
                        new SpellDataRegistryHolder(SpellRegistries.MANA_REND_SPELL, 1)
                ));
    }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        ...
        double manaRend = BigDecimal.valueOf(mc.player.getAttributeValue(PotatoEssentialsAttributes.MANA_REND)).setScale(2, RoundingMode.HALF_UP).doubleValue();
        ...
        tooltip.add(Component.literal("Mana Rend:")
                .withStyle(Style.EMPTY.withColor(TextColor.parseColor("#ed5e5e").getOrThrow())));
        tooltip.add(Component.literal("Deals [" + manaRend + "]% more damage")
                .withStyle(ChatFormatting.GRAY));
        tooltip.add(Component.literal("for every 100 mana the target has").withStyle(ChatFormatting.GRAY));
