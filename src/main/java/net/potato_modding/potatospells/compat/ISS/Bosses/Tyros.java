...
public class Tyros {
    ...
    private static void handleResistanceAttribute(EntityJoinLevelEvent event) {
        ...
        if (typeKey.getNamespace().equals("irons_spellbooks") && typeKey.getPath().equals("fire_boss")) {

            if (!ServerConfigs.TYROS_SWITCH.get()) {
                // Amethyst  attributes
                SpellPower += 1.2 * boss_mod;
                SchoolPower += 1.45 * boss_mod;
                Resist += 1.3 * boss_mod;
                FireRes += 2.0 * boss_mod;
                NatRes += 1.5 * boss_mod;
                EndRes += 0.8 * boss_mod;
                BloodRes += 1.35 * boss_mod;
                IceRes += 0.3 * boss_mod;
                LigRes += 1.2 * boss_mod;
                EldRes += 1.05 * boss_mod;
                HolyRes += 0.6 * boss_mod;
                BladeRes += 1.1 * boss_mod;
                AbyssRes += 0.25 * boss_mod;
                SoundRes += 0.75 * boss_mod;
                WindRes += 0.8 * boss_mod;
                Armor += 18 * spec_mod;
                Tough += 10 * spec_mod;
                Attack += 11 * spec_mod;
                ...
                BloodRes = ServerConfigs.TYROS_BLOOD_RESIST.get();
