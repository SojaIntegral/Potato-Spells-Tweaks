...
public class Harbinger {
    ...
    private static void handleResistanceAttributeCataclysm(EntityJoinLevelEvent event) {
        ...
        if (ModList.get().isLoaded("cataclysm") && mob.getType().is(PotatoTags.HARBINGER)) {

            if (!ServerConfigs.HARBINGER_SWITCH.get()) {
                // Amethyst  attributes
                Resist += 1.3 * boss_mod;
                FireRes += 1.8 * boss_mod;
                IceRes += 1.15 * boss_mod;
                HolyRes += 1.35 * boss_mod;
                NatRes += 1.65 * boss_mod;
                BldRes += 1.7 * boss_mod;
                EndRes += 1.1 * boss_mod;
                LigRes += 1.2 * boss_mod;
                ...
                BldRes = ServerConfigs.HARBINGER_BLOOD_RESIST.get();
