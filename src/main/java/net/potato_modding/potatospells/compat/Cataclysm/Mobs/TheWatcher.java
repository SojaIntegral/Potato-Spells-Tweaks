...
public class TheWatcher {
    ...
    private static void handleResistanceAttributeCataclysm(EntityJoinLevelEvent event) {
        ...
        if (typeKey.getNamespace().equals("cataclysm") && typeKey.getPath().equals("the_watcher")) {

            // Amethyst  attributes
            Resist += 1.1 * mob_mod;
            FireRes += 0.75 * mob_mod;
            IceRes += 1.3 * mob_mod;
            ...
            LigRes -= 0.05 * mob_mod;
            EldRes += 1.25 * mob_mod;
            AbyssRes += 0.6 * mob_mod;
            BladeRes += 1.4 * mob_mod;
            SoundRes += 0.7 * mob_mod;
            WindRes += 1.1 * mob_mod;
            Armor += 3 * spec_mod;
            Tough += 1 * spec_mod;
            Attack += 4 * spec_mod;
            ...
            // Updates mob attributes
            {
                ...
                // Fixed Attributes
                setIfNonNull((LivingEntity) mob, ALObjects.Attributes.ARMOR_PIERCE, 2 * mob_armor_pen);
                setIfNonNull((LivingEntity) mob, ALObjects.Attributes.ARMOR_SHRED, 2 * mob_armor_shred);
                setIfNonNull((LivingEntity) mob, ALObjects.Attributes.PROT_PIERCE, 2 * mob_prot_pen);
                setIfNonNull((LivingEntity) mob, ALObjects.Attributes.PROT_SHRED, 2 * mob_prot_shred);
