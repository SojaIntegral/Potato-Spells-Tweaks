...
public class Octohost {
    ...
    private static void handleResistanceAttributeCataclysm(EntityJoinLevelEvent event) {
        ...
        if (typeKey.getNamespace().equals("cataclysm") && typeKey.getPath().equals("octohost")) {
            ...
            NatRes += 2.0 * mob_mod;
            BloodRes += 3.0 * mob_mod;
            EndRes += 1.05 * mob_mod;
            LigRes -= 0.3 * mob_mod;
            EldRes += 2.5 * mob_mod;
            AbyssRes += 1.95 * mob_mod;
            BladeRes += 0.3 * mob_mod;
