...
public class Coralssus {
    ...
    private static void handleResistanceAttributeCataclysm(EntityJoinLevelEvent event) {
        ...
        if (typeKey.getNamespace().equals("cataclysm") && typeKey.getPath().equals("coralssus")) {

            // Amethyst  attributes
            Resist += 1.15 * mini_mod;
            FireRes += 1.35 * mini_mod;
            IceRes += 0.8 * mini_mod;
            HolyRes += 1.05 * mini_mod;
            NatRes += 1.25 * mini_mod;
            BloodRes += 0.95 * mini_mod;
            EndRes += 1.1 * mini_mod;
            LigRes += 0.35 * mini_mod;
            EldRes += 1.3 * mini_mod;
            AbyssRes += 1.45 * mini_mod;
            BladeRes += 1.05 * mini_mod;
            SoundRes += 0.85 * mini_mod;
            WindRes += 0.75 * mini_mod;
            Armor += 13 * spec_mod;
            Tough += 13 * spec_mod;
            Attack += 9 * spec_mod;
