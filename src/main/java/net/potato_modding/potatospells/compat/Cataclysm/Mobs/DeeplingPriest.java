...
public class DeeplingPriest {
    ...
    private static void handleResistanceAttributeCataclysm(EntityJoinLevelEvent event) {
        ...
        if (typeKey.getNamespace().equals("cataclysm") && typeKey.getPath().equals("deepling_priest")) {

            // Amethyst  attributes
            Resist += 1.2 * mob_mod;
            FireRes += 1.4 * mob_mod;
            IceRes += 0.9 * mob_mod;
            HolyRes += 1.35 * mob_mod;
            NatRes += 1.15 * mob_mod;
            BloodRes += 1.7 * mob_mod;
            EndRes += 1.1 * mob_mod;
            LigRes += 0.45 * mob_mod;
            EldRes += 0.8 * mob_mod;
            AbyssRes += 2.05 * mob_mod;
            BladeRes += 0.25 * mob_mod;
            SoundRes += 1.25 * mob_mod;
            WindRes += 0.85 * mob_mod;
            Armor += 2 * spec_mod;
            Tough += 5 * spec_mod;
            Attack += 6.5 * spec_mod;
