...
public class Cindaria {
    ...
    private static void handleResistanceAttributeCataclysm(EntityJoinLevelEvent event) {
        ...
        if (typeKey.getNamespace().equals("cataclysm") && typeKey.getPath().equals("cindaria")) {
            ...
            NatRes += 1.3 * mob_mod;
            BloodRes += 1.4 * mob_mod;
            EndRes += 1.05 * mob_mod;
            LigRes -= 0.85 * mob_mod;
            EldRes += 1.05 * mob_mod;
            AbyssRes += 1.75 * mob_mod;
            BladeRes -= 0.35 * mob_mod;
            SoundRes += 0.8 * mob_mod;
            WindRes += 0.6 * mob_mod;
            Armor += 0 * spec_mod;
            Tough += 0 * spec_mod;
            Attack += 10 * spec_mod;
