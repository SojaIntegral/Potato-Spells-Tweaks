...
public class IgnitedBerserk {
    ...
    private static void handleResistanceAttributeCataclysm(EntityJoinLevelEvent event) {
        ...
        if (typeKey.getNamespace().equals("cataclysm") && typeKey.getPath().equals("ignited_berserk")) {

            // Amethyst  attributes
            Resist += 0.75 * mob_mod;
            FireRes += 1.95 * mob_mod;
            IceRes += 0.55 * mob_mod;
            HolyRes += 1.05 * mob_mod;
            NatRes += 1.25 * mob_mod;
            BloodRes += 1.75 * mob_mod;
            EndRes += 1.05 * mob_mod;
            LigRes += 1.45 * mob_mod;
            EldRes += 1.05 * mob_mod;
            AbyssRes += 0.05 * mob_mod;
            BladeRes += 1.55 * mob_mod;
            SoundRes += 1.05 * mob_mod;
            WindRes += 0.9 * mob_mod;
            Armor += 6 * spec_mod;
            Tough += 9 * spec_mod;
            Attack += 9.5 * spec_mod;
