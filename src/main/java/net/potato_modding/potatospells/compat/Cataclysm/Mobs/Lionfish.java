...
public class Lionfish {
    ...
    private static void handleResistanceAttributeCataclysm(EntityJoinLevelEvent event) {
        ...
        if (typeKey.getNamespace().equals("cataclysm") && typeKey.getPath().equals("lionfish")) {
            ...
            NatRes += 0.75 * mob_mod;
            BloodRes += 0.6 * mob_mod;
            EndRes += 1.05 * mob_mod;
            LigRes -= 0.3 * mob_mod;
            EldRes += 1.2 * mob_mod;
            AbyssRes += 1.9 * mob_mod;
            BladeRes += 0.25 * mob_mod;
            SoundRes += 0.95 * mob_mod;
            WindRes += 0.75 * mob_mod;
            Armor += 4 * spec_mod;
            Tough += 2 * spec_mod;
            Attack += 7 * spec_mod;
