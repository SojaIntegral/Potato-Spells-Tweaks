...
public class Urchinkin {
    ...
    private static void handleResistanceAttributeCataclysm(EntityJoinLevelEvent event) {
        ...
        if (typeKey.getNamespace().equals("cataclysm") && typeKey.getPath().equals("urchinkin")) {

            // Amethyst  attributes
            Resist += 2.05 * mob_mod;
            FireRes += 2.05 * mob_mod;
            IceRes += 2.05 * mob_mod;
            ...
            BladeRes += 3.05 * mob_mod;
            SoundRes += 1.35 * mob_mod;
            WindRes += 0.3 * mob_mod;
            Armor += 10 * spec_mod;
            Tough += 10 * spec_mod;
            Attack += 5 * spec_mod;
