...
public class RoyalDraugr {
    ...
    private static void handleResistanceAttributeCataclysm(EntityJoinLevelEvent event) {
        ...
        if (typeKey.getNamespace().equals("cataclysm") && typeKey.getPath().equals("royal_draugr")) {

            // Amethyst  attributes
            Resist += 0.7 * mob_mod;
            FireRes += 0.6 * mob_mod;
            IceRes += 2.0 * mob_mod;
            HolyRes += 0.05 * mob_mod;
            NatRes += 1.05 * mob_mod;
            BloodRes += 1.85 * mob_mod;
            EndRes += 1.1 * mob_mod;
            LigRes += 0.95 * mob_mod;
            EldRes += 1.15 * mob_mod;
            AbyssRes += 1.05 * mob_mod;
            BladeRes += 1.6 * mob_mod;
            SoundRes += 0.3 * mob_mod;
            WindRes += 1.5 * mob_mod;
            Armor += 15 * spec_mod;
            Tough += 12 * spec_mod;
            Attack += 10 * spec_mod;
