...
public class EliteDraugr {
    ...
    private static void handleResistanceAttributeCataclysm(EntityJoinLevelEvent event) {
        ...
        if (typeKey.getNamespace().equals("cataclysm") && typeKey.getPath().equals("elite_draugr")) {

            // Amethyst  attributes
            Resist += 0.85 * mob_mod;
            FireRes += 0.75 * mob_mod;
            IceRes += 1.95 * mob_mod;
            HolyRes += 0.15 * mob_mod;
            NatRes += 1.4 * mob_mod;
            BloodRes += 2.15 * mob_mod;
            EndRes += 1.05 * mob_mod;
            LigRes += 0.85 * mob_mod;
            EldRes += 1.25 * mob_mod;
            AbyssRes += 1.15 * mob_mod;
            BladeRes += 1.5 * mob_mod;
            SoundRes += 0.4 * mob_mod;
