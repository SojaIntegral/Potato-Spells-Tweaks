...
public class SummonerPet {
    ...
    private static void handleResistanceAttribute(FinalizeSpawnEvent event) {
        ...
        if (typeKey.getNamespace().equals("alshanex_familiars") && typeKey.getPath().equals("summoner_pet")
            ...
            SpellPower += 1.05 * mob_mod * attrVar[0];
            Resist += 1.25 * mob_mod * attrVar[1];
            FireRes += 0.8 * mob_mod * attrVar[2];
            IceRes += 1.2 * mob_mod * attrVar[2];
            HolyRes += 0.5 * mob_mod * attrVar[2];
            NatRes += 1.5 * mob_mod * attrVar[2];
            EvokeRes += 1.5 * mob_mod * attrVar[2];
            BloodRes += mob_mod * attrVar[2];
            EndRes += 1.2 * mob_mod * attrVar[2];
            LigRes += 1.2 * mob_mod * attrVar[2];
            EldRes += 1.5 * mob_mod * attrVar[2];
            AbyssRes += mob_mod * attrVar[2];
            BladeRes += 0.5 * mob_mod * attrVar[2];
            SoundRes += 0.5 * mob_mod * attrVar[2];
            WindRes += mob_mod * attrVar[2];
            Armor += 5 * spec_mod * attrVar[3];
            Tough += 5 * spec_mod * attrVar[4];
            ...
            // Updates mob attributes
            {
                ...
                setIfNonNull(mob, ALObjects.Attributes.CRIT_CHANCE, 0.2 * attrVar[8]);
                setIfNonNull(mob, ALObjects.Attributes.CRIT_DAMAGE, 1.45 * attrVar[9]);
            }
