...
public class BardPet {
    ...
    private static void handleResistanceAttribute(FinalizeSpawnEvent event) {
        ...
        if (typeKey.getNamespace().equals("alshanex_familiars") && typeKey.getPath().equals("bard_pet")
            ...
            SpellPower += mob_mod * attrVar[0];
            Resist += 1.3 * mob_mod * attrVar[1];
            FireRes += mob_mod * attrVar[2];
            IceRes += 1.2 * mob_mod * attrVar[2];
            HolyRes += 1.5 * mob_mod * attrVar[2];
            NatRes += 0.5 * mob_mod * attrVar[2];
            ...
            BladeRes += 1.5 * mob_mod * attrVar[2];
            SoundRes += 1.5 * mob_mod * attrVar[2];
            WindRes += 0.8 * mob_mod * attrVar[2];
            Armor += 4 * spec_mod * attrVar[3];
            Tough += 4 * spec_mod * attrVar[4];
            ...
            // Updates mob attributes
            {
                ...
                setIfNonNull(mob, ALObjects.Attributes.CRIT_DAMAGE, 1.4 * attrVar[9]);
            }
