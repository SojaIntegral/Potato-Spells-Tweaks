package net.potato_modding.potatospells.resistances.compat.special_bosses;
...
public class TheHarbinger {
    ...
    private static void handleResistanceAttributeCataclysm(EntityJoinLevelEvent event) {
        ...
        if (typeKey.getNamespace().equals("cataclysm") && typeKey.getPath().equals("the_harbinger")) {

            if (!ServerConfigs.HARBINGER_SWITCH.get()) {
                // Amethyst  attributes
                Resist += 1.3 * boss_mod;
                FireRes += 1.05 * boss_mod;
                ...
                Armor = ServerConfigs.HARBINGER_ARMOR.get();
                Tough = ServerConfigs.HARBINGER_TOUGHNESS.get();
                Attack = ServerConfigs.HARBINGER_ATTACK.get();
                Resist = ServerConfigs.HARBINGER_RESIST.get();
                FireRes = ServerConfigs.HARBINGER_FIRE_RESIST.get();
                NatRes = ServerConfigs.HARBINGER_NATURE_RESIST.get();
                EndRes = ServerConfigs.HARBINGER_ENDER_RESIST.get();
                BloodRes = ServerConfigs.HARBINGER_BLOOD_RESIST.get();
                IceRes = ServerConfigs.HARBINGER_ICE_RESIST.get();
                LigRes = ServerConfigs.HARBINGER_LIGHTNING_RESIST.get();
                EldRes = ServerConfigs.HARBINGER_ELDRITCH_RESIST.get();
                HolyRes = ServerConfigs.HARBINGER_HOLY_RESIST.get();
                BladeRes = ServerConfigs.HARBINGER_BLADE_RESIST.get();
                AbyssRes = ServerConfigs.HARBINGER_ABYSSAL_RESIST.get();
                SoundRes = ServerConfigs.HARBINGER_MUSIC_RESIST.get();
                WindRes = ServerConfigs.HARBINGER_WIND_RESIST.get();
            }
            ...
            // Updates mob attributes
            {
                ...
                // Fixed Attributes
                setIfNonNull((LivingEntity) mob, ALObjects.Attributes.ARMOR_PIERCE, 2 * boss_armor_pen);
                setIfNonNull((LivingEntity) mob, ALObjects.Attributes.ARMOR_SHRED, 2 * boss_armor_shred);
                setIfNonNull((LivingEntity) mob, ALObjects.Attributes.PROT_PIERCE, 2 * boss_prot_pen);
                setIfNonNull((LivingEntity) mob, ALObjects.Attributes.PROT_SHRED, 2 * boss_prot_shred);
                setIfNonNull((LivingEntity) mob, ALObjects.Attributes.CRIT_CHANCE, 1);
                setIfNonNull((LivingEntity) mob, ALObjects.Attributes.CRIT_DAMAGE, 1.75);
            }
