...
@AutoSpellConfig
public class ValianceSpell extends AbstractSpell {
    private final ResourceLocation spellId = ResourceLocation.fromNamespaceAndPath(PotatoSpells.MOD_ID, "valiance");

    @Override
    public List<MutableComponent> getUniqueInfo(int spellLevel, LivingEntity caster) {
        return List.of(
                Component.translatable("ui.irons_spellbooks.damage", Utils.stringTruncation(getDamage(spellLevel, caster), 2)),
                Component.translatable("ui.irons_spellbooks.effect_length", Utils.timeFromTicks(300, 1))
                );
    }
            ...
            if (targetEntity instanceof LivingEntity && targetEntity.isAlive() && entity.isPickable() && targetEntity.position().subtract(entity.getEyePosition()).dot(forward) >= 0 && entity.distanceToSqr(targetEntity) < radius * radius && Utils.hasLineOfSight(level, entity.getEyePosition(), targetEntity.getBoundingBox().getCenter(), true)) {
                ...
                if (offsetVector.dot(forward) >= 0) {
                    ...
                    if (DamageSources.applyDamage(targetEntity, (float)(getDamage(spellLevel, entity)), damageSource)) {
                        ...
                        entity.addEffect(new MobEffectInstance(PotatoEffects.VALIANCE_EFFECT,
                                300,
                                0,
                                true,
                                false,
    ...
    private float getDamage(int spellLevel, LivingEntity caster) {
        if(caster != null) {
            double physicalDamage = caster.getAttributeValue(Attributes.ATTACK_DAMAGE) * 0.5;

            return (float) ((0.5 + 0.65 * getSpellPower(spellLevel, caster)) * (physicalDamage - 0.5) * (1 + caster.getAttributeValue(AttributeRegistry.MAX_MANA) * 0.001f));
        }
        else return 0;
    }
