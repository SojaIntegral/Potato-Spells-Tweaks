...
@AutoSpellConfig
public class AcheronSpell extends AbstractSpell {
    private final ResourceLocation spellId = ResourceLocation.fromNamespaceAndPath(PotatoSpells.MOD_ID, "acheron");
            ...
            if (targetEntity instanceof LivingEntity && targetEntity.isAlive() && entity.isPickable() && targetEntity.position().subtract(entity.getEyePosition()).dot(forward) >= 0 && entity.distanceToSqr(targetEntity) < radius * radius && Utils.hasLineOfSight(level, entity.getEyePosition(), targetEntity.getBoundingBox().getCenter(), true)) {
                ...
                if (offsetVector.dot(forward) >= 0) {
                    ...
                    if (DamageSources.applyDamage(targetEntity, (float)(getDamage(spellLevel, entity)), damageSource)) {
                        ...
                        ((LivingEntity) targetEntity).addEffect(new MobEffectInstance(PotatoEffects.ACHERON_EFFECT,
                                100,
                                0,
                                true,
                                true,
    ...
    private float getDamage(int spellLevel, LivingEntity caster) {
        if(caster != null) {
            double physicalDamage = caster.getAttributeValue(Attributes.ATTACK_DAMAGE) * 0.15;

            return (float) (0.35 * getSpellPower(spellLevel, caster) * physicalDamage * (caster.getAttributeValue(AttributeRegistry.MAX_MANA) * 0.001f));
        }
        else return 0;
    }
